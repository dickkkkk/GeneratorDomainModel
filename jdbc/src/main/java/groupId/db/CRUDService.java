...
public class CRUDService {
    ...
    public Indicators getIndicator(Long playerId) {
        Indicators indicator = null;
        try (PreparedStatement pr2 = connection.prepareStatement("SELECT * FROM indicators WHERE player_id = ?")) {
            pr2.setLong(1, playerId);
            ResultSet rs2 = pr2.executeQuery();
            indicator = getIndicatorFromRs(rs2);
        } catch (SQLException throwables) {
            throwables.printStackTrace();
        }
        return indicator;
    ...
    public Achievement getAchievement(Long id) {
        ...
        try (PreparedStatement pr = connection.prepareStatement("SELECT * FROM achievements WHERE id = ?")) {
            pr.setLong(1, id);
            ResultSet rs = pr.executeQuery();
            while (rs.next()) {
                long playerId = rs.getLong("player_id");
                String name = rs.getString("name");
                achievement = new Achievement(id, playerId, name);
            }
        } catch (SQLException throwables) {
            throwables.printStackTrace();
        }
    ...
    public boolean savePlayer(Player player) {
        if (searchPlayerId(player.getPlayerId())) {
            return false;
        }
        try (
    ...
    public boolean createIndicators(Indicators indicators) {
        if (!searchPlayerId(indicators.getPlayerId())) {
            return false;
        }
        try {
    ...
    public boolean createAchievement(Achievement achievement) {
        if (searchPlayerId(achievement.getPlayerId())) {
            return false;
        }
        try {
    ...
    public boolean createItem(Item item) {
        if (searchPlayerId(item.getPlayerId())) {
            return false;
        }
        try {
    ...
    public boolean createCurrency(Currency currency) {
        if (searchPlayerId(currency.getPlayerId())) {
            return false;
        }
        try {
            PreparedStatement preparedStatement = connection.prepareStatement(insertPlayers[4]);
    ...
    public boolean createEffect(Effect effect) {
        if (searchPlayerId(effect.getPlayerId())) {
            return false;
        }
        try {
            PreparedStatement preparedStatement = connection.prepareStatement(insertPlayers[5]);
    ...
    public boolean createSpell(Spell spell) {
        if (searchPlayerId(spell.getPlayerId())) {
            return false;
        }
        try {
            PreparedStatement preparedStatement = connection.prepareStatement(insertPlayers[6]);
    ...
    public boolean searchPlayerId(Long id) {
        HashMap<Integer, Integer> map = new HashMap<>();
        try (PreparedStatement preparedStatement = connection.prepareStatement("SELECT player_id FROM players")) {
    ...
    public boolean searchPlayerIdInTableIndicators(Long playerId) {
        boolean flag = false;
        String request = "SELECT * FROM indicators WHERE player_id=?";
    ...
    public boolean searchIdInPlayerTable(Long id) {
        boolean flag = false;
        String request = "SELECT * FROM players WHERE id=?";
    ...
    public boolean searchIdInAchievementsTable(Long id) {
        boolean flag = false;
        String request = "SELECT * FROM achievements WHERE id=?";
        try (PreparedStatement entities = connection.prepareStatement(request)) {
            entities.setLong(1, id);
            ResultSet rs = entities.executeQuery();
            while (rs.next()) {
    ...
    public boolean searchIdInCurrencyTable(Long id) {
        boolean flag = false;
        String request = "SELECT * FROM currencies WHERE id=?";
        try (PreparedStatement entities = connection.prepareStatement(request)) {
            entities.setLong(1, id);
            ResultSet rs = entities.executeQuery();
            while (rs.next()) {
    ...
    public boolean searchIdInEffectTable(Long id) {
        boolean flag = false;
        String request = "SELECT * FROM effects WHERE id=?";
        try (PreparedStatement entities = connection.prepareStatement(request)) {
            entities.setLong(1, id);
            ResultSet rs = entities.executeQuery();
            while (rs.next()) {
    ...
    public boolean searchIdInSpellTable(Long id) {
        boolean flag = false;
        String request = "SELECT * FROM spells WHERE id=?";
        try (PreparedStatement entities = connection.prepareStatement(request)) {
            entities.setLong(1, id);
            ResultSet rs = entities.executeQuery();
            while (rs.next()) {
